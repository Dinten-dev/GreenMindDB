...
def test_public_observation_session_flow(
    client: TestClient, db, tenant_admin_token, setup_test_data
):
    ...
    res = client.post("/api/v1/public/observe/session", json={"public_id": public_id})
    assert res.status_code == 200
    session_data = res.json()
    session_token = session_data["session_token"]

    # 4. Get Plant Context
    res = client.get(f"/api/v1/public/observe/session/{session_token}/context")
    assert res.status_code == 200
    context = res.json()
...
def test_invalid_session_fails(client: TestClient):
    res = client.post(
        ...
        json={"wellbeing_score": 3, "plant_condition": "medium"},
    )
    assert res.status_code == 401


def test_revoked_access_fails(client: TestClient, db, tenant_admin_token, setup_test_data):
    zone = setup_test_data["zone"]
