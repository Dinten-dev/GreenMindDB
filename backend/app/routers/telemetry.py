    ...
    for sample in payload.samples:
        ...
        if chan_res:
            ...
            try:
                ...
                # Fallback: channel creation failed, will re-fetch below
                logging.getLogger(__name__).debug("Channel upsert fallback for species=%s metric=%s", species_id, metric_id)
...
def download_bioelectric_wav(
    ...
    if not bioelectric_metric:
        return StreamingResponse(
            ...
            headers={"Content-Disposition": "attachment; filename=no_bioelectric_data.wav"}
        )
