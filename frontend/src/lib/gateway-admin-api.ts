...
export async function toggleConfigRelease(
  id: string,
  isActive: boolean
): Promise<GatewayConfigRelease> {
  return gwFetch<GatewayConfigRelease>(
    `/gateway-config-releases/${id}/status?is_active=${isActive}`,
    {
      method: 'PATCH',
    }
  );
...
export async function initiateRollback(gatewayId: string, targetVersion?: string): Promise<void> {
