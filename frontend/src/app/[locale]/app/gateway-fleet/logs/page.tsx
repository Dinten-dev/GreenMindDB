          ...
          <table className="w-full text-sm" id="update-logs-table">
            <thead>
              <tr className="border-b border-black/[0.06]">
                {['Gateway', 'Typ', 'Von', 'Nach', 'Status', 'Fehler', 'Gestartet', 'Beendet'].map(
                  (h) => (
                    <th
                      key={h}
              ...
              {loading ? (
                ...
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-black/[0.015] transition-colors">
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-gray-800">{log.to_version}</td>
                    <td className="px-4 py-3">
