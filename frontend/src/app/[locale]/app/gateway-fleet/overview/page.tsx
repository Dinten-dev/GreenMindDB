...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium border ${styles[status] || styles.offline}`}
    >
      <span
        className={`w-1.5 h-1.5 rounded-full ${status === 'online' ? 'bg-emerald-500' : 'bg-gray-400'}`}
      />
      {status}
    </span>
  );
...
function UpdateStatusBadge({ download, apply }: { download: string | null; apply: string | null }) {
  const effective =
    apply && apply !== 'none' ? apply : download && download !== 'none' ? download : null;
  ...
  return (
    <span
      className={`px-2 py-0.5 rounded-md text-xs font-medium ${styles[effective] || 'text-gray-500 bg-gray-50'}`}
    >
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          {
            label: 'Maintenance',
            value: fleet.filter((g) => g.maintenance_mode).length,
                ...
                fleet.map((gw) => (
                  <tr key={gw.id} className="hover:bg-black/[0.015] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {gw.blocked && <span className="ml-1 text-xs text-red-500">⊘ blocked</span>}
                    </td>
                    <td className="px-4 py-3 text-xs">
                      ...
                      {gw.desired_app_version && gw.desired_app_version !== gw.app_version && (
                        <span className="block text-amber-500 mt-0.5">
                          → {gw.desired_app_version}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-xs font-mono">{gw.config_version || '—'}</td>
                    <td className="px-4 py-3">
