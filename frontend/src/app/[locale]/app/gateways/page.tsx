...
export default function GatewaysPage() {
  ...
  const loadData = async () => {
    try {
      const [gws, zs] = await Promise.all([apiListGateways(), apiListZones()]);
      setGateways(gws);
      setZones(zs);
    } catch (err) {
      console.error(err);
            ...
            {!pairingCode ? (
              ...
              <div className="text-center space-y-4">
                ...
                <p className="text-xs text-gray-400">
                  Gültig bis {new Date(pairingCode.expires_at).toLocaleTimeString('de-CH')} (10
                  Minuten)
                </p>
                <button
                  onClick={() => {
            ...
            <div key={gw.id} className="glass-card accent-glow p-6">
              ...
              <div className="grid grid-cols-3 gap-2 text-sm">
                ...
                <div className="bg-white/40 rounded-xl px-2.5 py-1.5 border border-black/[0.03]">
                  ...
                  <p className="font-semibold text-gray-800 text-xs font-mono truncate">
                    {gw.local_ip || '–'}
                  </p>
                </div>
                <div className="bg-white/40 rounded-xl px-2.5 py-1.5 border border-black/[0.03]">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white/80 rounded-2xl border border-white/50 shadow-2xl backdrop-blur-xl p-6 max-w-sm w-full animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-sm text-center text-gray-500 mb-6">
              Damit wird auch die Verbindung aller zugehörigen Sensoren getrennt. Die Sensordaten
