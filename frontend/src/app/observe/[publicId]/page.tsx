...
import { apiCreateSession, apiGetPlantContext, apiCreateEvaluation } from '@/lib/observe-api';
...
export default function PlantEvaluationPage() {
  ...
  const isStepValid = (): boolean => {
    switch (step) {
      ...
      case 2:
        return colorRaw !== null;
      case 3:
        return structureRaw !== null;
      case 4:
        return growthRaw !== null;
      case 5:
        return anomaliesRaw.length > 0;
      case 6:
        return waterRaw !== null;
      default:
        return false;
    }
  };
      ...
      <header className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-4 py-4 shadow-lg">
        <div className="max-w-md mx-auto flex items-center justify-between">
          <div>
            ...
            {context && <p className="text-emerald-100 text-xs mt-0.5">{context.name}</p>}
          </div>
      ...
      <main className="flex-1 w-full max-w-md mx-auto px-4 py-6 flex flex-col">
        ...
        {phase === 'EVAL' && (
          <div>
            ...
            {step === 1 && (
              <StepCard title="Gesamtzustand" subtitle="Wie geht es der Pflanze insgesamt?">
                ...
                {overallScore !== null && (
                  <div
                    className={`mt-4 text-center py-2 rounded-xl text-sm font-semibold ${OVERALL_COLORS[overallScore]} bg-opacity-10`}
                  >
                ...
                <NavButtons onNext={goNext} nextDisabled={!isStepValid()} />
              </StepCard>
            )}

            {/* Step 2: Leaf Color */}
            {step === 2 && (
              <StepCard title="Blattfarbe" subtitle="Welche Farbe haben die Blätter?">
                ...
                <NavButtons onBack={goBack} onNext={goNext} nextDisabled={!isStepValid()} />
              </StepCard>
            )}

            {/* Step 3: Leaf Structure */}
            {step === 3 && (
              <StepCard title="Blattstruktur / Spannung" subtitle="Wie fühlen sich die Blätter an?">
                <div className="space-y-2">
            ...
            {/* Step 4: Growth */}
            {step === 4 && (
              <StepCard title="Wachstum" subtitle="Wie entwickelt sich die Pflanze?">
                <div className="space-y-2">
            ...
            {/* Step 6: Water State */}
            {step === 6 && (
              <StepCard title="Wasserzustand" subtitle="Wie schätzen Sie die Feuchtigkeit ein?">
                <div className="space-y-2">
        ...
        {phase === 'DONE' && (
          <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <h2 className="text-2xl font-bold text-gray-800">Danke – Bewertung gespeichert</h2>
            <p className="text-gray-500 text-sm max-w-xs mx-auto">
              Ihre Pflanzenbewertung wurde erfolgreich übermittelt und wird für die GreenMind
              Forschung verwendet. Sie können dieses Fenster nun schliessen.
            </p>
          </div>
        )}
      </main>
